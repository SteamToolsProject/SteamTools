...
pub fn format_catalog_lua(app_id: AppId, bundle: &CatalogBundle) -> String {
    ...
    if let Some(key) = bundle.depot_keys.get(&app_id) {
        if key.len() == 64 {
            out.push_str(&format!("addappid({app_id}, 0, \"{key}\")\n"));
        }
    }
    // 其它 app 的 key (少见, 一并写出)
    for (&id, key) in &bundle.depot_keys {
        if id != app_id && key.len() == 64 {
            out.push_str(&format!("addappid({id}, 0, \"{key}\")\n"));
        }
    }
    for (&id, &token) in &bundle.access_tokens {
...
mod tests {
    ...
    fn format_lua_contains_addappid_and_key() {
        ...
        b.apps.push(10);
        b.depot_keys.insert(10, "a".repeat(64));
        b.manifests.insert(
            11,
    ...
    fn keeps_depot_key_from_provider() {
        let (_root, state, _out) = add_app_42();
        assert_eq!(
            state.with_rules(|r: &AppRules| r.depot_key(42).map(str::len)),
            Some(64)
        );
    }
