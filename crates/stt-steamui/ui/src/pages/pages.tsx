...
export function SourcesPage({ snapshot }: { snapshot: ConfigSnapshot }) {
  ...
  return <><Lead title="数据源" description="选择目录、清单和日志使用的来源" /><FormRow title="Catalog 源" detail={snapshot.catalog_status} control={<Segmented label="Catalog 源" values={snapshot.catalog_modes} current={snapshot.catalog_mode} labels={CATALOG_LABELS} onChange={(value) => emitIntent({ kind: "set_catalog_mode", value })} />} /><div className="stt-row"><div className="stt-row-main"><div className="stt-row-title">Catalog URL 模板</div><div className="stt-row-detail">必须包含 {"{app_id}"}, 由宿主校验后保存</div>{catalogError ? <div className="stt-field-error" id="catalog-url-error">{catalogError}</div> : null}</div><div className="stt-actions"><input className={`stt-input${catalogError ? " stt-input-error" : ""}`} aria-invalid={catalogError ? "true" : undefined} aria-describedby={catalogError ? "catalog-url-error" : undefined} placeholder="https://catalog.example/v1/{app_id}" value={catalogDraftValue} onInput={(event) => { setCatalogBusy(false); setCatalogPending(null); setCatalogError(""); setCatalogDraft((event.currentTarget as HTMLInputElement).value); }} onKeyDown={(event) => { if (event.key === "Enter") saveCatalog(); }} /><Button disabled={catalogBusy} onClick={saveCatalog}>{catalogBusy ? "保存中" : "保存 URL"}</Button></div></div><FormRow title="Manifest 请求码源" detail="仅用于下载阶段的 request code" control={<Segmented label="Manifest 请求码源" values={snapshot.manifest_sources} current={snapshot.manifest_url} onChange={(value) => emitIntent({ kind: "set_manifest_url", value })} />} /><FormRow title="日志级别" detail="写进 steamtools/host.log" control={<Segmented label="日志级别" values={snapshot.log_levels} current={snapshot.log_level} onChange={(value) => emitIntent({ kind: "set_log_level", value })} />} /></>;
}
...
export function AppsPage({ snapshot }: { snapshot: ConfigSnapshot }) {
  ...
  if (!snapshot.managed.length) return <><Lead title="管理列表" description="由 SteamTools 写入 config/lua 的应用" /><EmptyState title="还没有入库的应用" detail="打开 Steam 商店详情页, 使用入库按钮添加应用" /></>;
  return <><Lead title="管理列表" description={`${snapshot.managed.length} 个应用由 SteamTools 管理`} />{snapshot.managed.map((id) => <FormRow key={id} title={names.get(id) || `Steam 应用 ${id}`} detail={`AppID ${id} · config/lua/stt_${id}.lua`} control={<div className="stt-actions">{confirming === id ? <><Button onClick={() => setConfirming(null)}>取消</Button><Button danger disabled={busy[id] === "remove"} onClick={() => { setBusy((old) => ({ ...old, [id]: "remove" })); emitIntent({ kind: "remove_app", app_id: id }); }}>{busy[id] === "remove" ? "移除中" : "确认移除"}</Button></> : <><Button disabled={busy[id] === "refresh"} onClick={() => { setBusy((old) => ({ ...old, [id]: "refresh" })); emitIntent({ kind: "refresh_app", app_id: id }); }}>{busy[id] === "refresh" ? "刷新中" : "刷新清单"}</Button><Button onClick={() => setConfirming(id)}>移除</Button></>}</div>} />)}</>;
}
...
export function LuaPage({ snapshot }: { snapshot: ConfigSnapshot }) {
  ...
  return <><Lead title="脚本目录" description="按顺序加载配置目录中的脚本" /><FormRow title={snapshot.lua_dir} detail="总是加载, 不可移除" mono control={<span className="stt-spacer-value">默认</span>} />{snapshot.lua_paths.map((path) => { const isPending = pendingRemove === path; const isConfirming = confirming === path; return <FormRow key={path} title={path} mono control={isConfirming ? <div className="stt-actions"><Button onClick={() => setConfirming(null)}>取消</Button><Button danger disabled={isPending} onClick={() => { setPendingRemove(path); emitIntent({ kind: "remove_lua_path", value: path }); }}>{isPending ? "移除中" : "确认移除"}</Button></div> : <Button danger disabled={Boolean(pendingRemove)} onClick={() => setConfirming(path)}>移除</Button>} />; })}<div className="stt-row"><div className="stt-row-main"><div className="stt-row-title">添加 Lua 目录</div><div className="stt-row-detail">需要是存在的绝对路径</div></div><div className="stt-actions"><input className="stt-input" placeholder="再加一个 lua 目录的绝对路径" value={draft} onInput={(event) => { setBusy(false); setPendingPath(null); setDraft((event.currentTarget as HTMLInputElement).value); }} onKeyDown={(event) => { if (event.key === "Enter") submit(); }} /><Button disabled={busy} onClick={submit}>{busy ? "添加中" : "添加"}</Button></div></div></>;
}

export function StatusPage({ snapshot }: { snapshot: ConfigSnapshot }) {
  const enabled = snapshot.tools.filter((tool) => tool.enabled).length;
  const rows: Array<[string, string, string]> = [["连接方式", snapshot.channel || "等待连接", "SteamTools 与客户端界面之间的连接"], ["已加载应用", String(snapshot.owned_count), "当前配置中可识别的应用数量"], ["配置变更", `第 ${snapshot.epoch} 次`, "每次保存配置后更新"], ["已启用工具", `${enabled} / ${snapshot.tools.length}`, "当前打开的功能数量"], ["版本", `v${snapshot.version || "?"}`, ""]];
